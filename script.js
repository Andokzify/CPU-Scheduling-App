...
function addJob() {
  ...
  void box.offsetWidth; // This "magic" line resets the animation
...
function updateJobCounter() {
  const counter = document.querySelector('.number-box');
  ...
  // Trigger a "reflow" so the browser notices the removal
...
function runSRTF(jobs) {
  ...
  // Give every job a "remaining" counter that starts equal to its burst
  // and counts DOWN as it runs. We copy so we don't change the original.
  ...
  let clock       = 0;
  let done        = 0;
  let lastJobId   = null; // tracks who ran in the previous tick

  // Keep running until all jobs are finished
  while (done < jobs.length) {

    // Find all jobs that have arrived and still have work left
    let available = tasks.filter(j => j.arrival <= clock && j.remaining > 0);

    if (available.length === 0) {
      // No job is ready — CPU is idle, jump to next arrival
      let nextArrival = Math.min(...tasks.filter(j => j.remaining > 0).map(j => j.arrival));
      ganttBlocks.push({ id: "IDLE", start: clock, end: nextArrival });
      clock     = nextArrival;
      lastJobId = null;
      continue;
    ...
    // Pick the job with the LEAST remaining time
    let best = available[0];
    for (let i = 1; i < available.length; i++) {
      if (available[i].remaining < best.remaining) best = available[i];
    }

    // If same job as last tick → extend its last Gantt block by 1
    // If different job → start a new Gantt block
    if (best.id === lastJobId) {
      ganttBlocks[ganttBlocks.length - 1].end = clock + 1;
    } else {
      ganttBlocks.push({ id: best.id, start: clock, end: clock + 1 });
    }

    // Run this job for exactly 1 tick
    best.remaining--;
    clock++;
    lastJobId = best.id;

    // If this job just finished, count it
